...
function compareObjects(obj1, obj2) {
  ...
  if (
    ...
    keys1.every((key, i) => key === keys2[i] && obj1[key] === obj2[key])
  ) {
    result = true;
  }
...
function makeWord(lettersObject) {
  return Object.keys(lettersObject)
    .map((char) => lettersObject[char].map((pos) => [pos, char]))
    .flat()
    .sort(([a], [b]) => a - b)
    .map(([, char]) => char)
    .join('');
...
class Selector {
  ...
  combine(selector1, combinator, selector2) {
    this.parts = [...selector1.parts, ` ${combinator} `, ...selector2.parts];
    return this;
  }
